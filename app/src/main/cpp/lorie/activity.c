...
static void nativeInit(JNIEnv *env, jobject thiz) {
    ...
    if (!Charset.self) {
        ...
        MainActivity.self = FindClassOrDie(env,  "com/alevde/MainActivity");
        MainActivity.getInstance = FindMethodOrDie(env, MainActivity.self, "getInstance", "()Lcom/alevde/MainActivity;", JNI_TRUE);
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, __unused void *reserved) {
    ...
    jclass cls = (*env)->FindClass(env, "com/alevde/LorieView");
