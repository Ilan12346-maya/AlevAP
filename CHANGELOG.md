...
- **Binary Patching Engine:** Added automatic binary-level string replacement (`com.termux` -> `com.alevde`) in `bundle_x11.sh` to fix hardcoded paths in pre-compiled binaries like `htop` and `bash`.
